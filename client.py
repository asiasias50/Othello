...
class Client:  # Class formats and sends requests to the server, receives and decodes responses
    ...
    def save_game(self, puzzle, finished, player1_id, player2_id, game_sequence, starting_player):  # Sending a request to store the game state with provided parameters
        game_sequence_str = ""
        if puzzle:
        ...
        self.__client.send(bytes(self.SAVE_GAME + dumps((puzzle, finished, player1_id, player2_id, game_sequence_str)), "utf-8"))
        return loads(self.__client.recv(1024).decode('utf-8'))
...
if __name__ == '__main__':
    c = Client()
    print(c.save_game(True, False, 1, None, [["B", "W"], ["W", "B"]], "B"))