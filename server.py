...
class Server:  # Class acts as a stand alone application, retrieving requests from clients, quering requests to the MySQL database and sending appropriate responces
    CREATE_ACCOUNT = "ca"
    SIGN_IN = "si"
    SAVE_GAME = "sg"
    RETRIEVE = "re"
    ...
    def __init__(self):
        self.__procedure_list = {self.CREATE_ACCOUNT: self.__create_account, self.SIGN_IN: self.__sign_in, self.SAVE_GAME: self.__save_game, self.RETRIEVE: self.__retrieve_game, self.SHOW_PLAYED_GAMES: self.__show_played_games}
    ...
    def __create_account(self, arguments, client):  # Querying MySQL database to create new user account
        ...
        if self.__cursor.fetchall()[0][0] > 0:
            ...
            message = True
            self.__cursor.execute(f'INSERT INTO user_information (USERNAME, PASSWORD_HASH) VALUES ("{username}", "{password}")')
    ...
    def __save_game(self, arguments, client):  # Querying either an update to existing game save or to create a new entry for game save
        puzzle, finished, player1_id, player2_id, game_sequence_str = arguments
        if player2_id is None:
            player2_id = "NULL"
        else:
            player2_id = f'"{player2_id}"'
        self.__cursor.execute(f'SELECT game_id FROM games_played WHERE player1_id = "{player1_id}" AND player2_id = {player2_id}')
        game_id = self.__cursor.fetchall()
        if len(game_id) > 0:
            self.__cursor.execute(f'UPDATE game_information SET finished = "{int(finished)}", game_sequence = {game_sequence_str} WHERE game_id = {game_id[0][0]}')
        else:
            self.__cursor.execute(f'INSERT INTO game_information (player1_id, player2_id) VALUES ("{puzzle}", "{finished}", "{game_sequence_str}")')
            self.__cursor.execute(f'INSERT INTO games_played (player1_id, player2_id) VALUES ("{player1_id}", {player2_id})')
    ...
    def __show_played_games(self, arguments, client):
        ...
        self.__cursor.execute(f'SELECT * FROM game_information INNER JOIN games_played ON game_information.game_id = games_played.game_id WHERE player1_id = "{player_id}" OR player2_id = "{player_id}" ORDER BY game_id DESC')
